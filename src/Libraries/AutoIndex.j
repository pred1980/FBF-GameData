...
//map. Disable it by removing the exclamation to remove the delay while saving.
	...
	struct AutoIndex
		...
		static method getIndexDebug takes unit u returns integer
				...
				elseif idunit[getIndex(u)] != u and GetIssuedOrderId() != 852056 then
					debug call BJDebugMsg("AutoIndex error: " + GetUnitName(u) + " is a filtered unit.")
				endif
			return getIndex(u)
		endmethod //If debug mode is enabled, use the getIndex method that shows errors.
		
		static method getUnit takes integer index returns unit
			return idunit[index]
		endmethod
    ...
    function GetUnitId takes unit u returns integer
        static if IS_DEBUG_MODE then             //If debug mode is enabled...
    ...
    function GetUnitById takes integer index returns unit
        return AutoIndex.getUnit(index)
    endfunction
