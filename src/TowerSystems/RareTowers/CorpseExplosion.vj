scope CorpseExplosion initializer init
	...
	private struct CorpseExplosion
		...
		private static method debuffUnit takes unit tower, unit summonedUnit, unit u, integer level returns nothing
			...
			if ms > 0 then
				if level == 1 then
					call AddUnitBonus(u, BONUS_MOVEMENT_SPEED, R2I(-1.*ms*SLOW_MULTIPLIER_LEVEL1))
				elseif level == 2 then
					call AddUnitBonus(u, BONUS_MOVEMENT_SPEED, R2I(-1.*ms*SLOW_MULTIPLIER_LEVEL2))
				elseif level == 3 then
					call AddUnitBonus(u, BONUS_MOVEMENT_SPEED, R2I(-1.*ms*SLOW_MULTIPLIER_LEVEL3))
				else
