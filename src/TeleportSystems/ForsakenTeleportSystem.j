scope ForsakenTeleportSystem initializer init
    ...
    struct ForsakenTeleport
		...
		static method onMoveTeleporter takes nothing returns nothing
			...
			set x = GetRectCenterX(TELEPORT_RECTS[PLACE_BY_ROUND[i][0]])
			set y = GetRectCenterY(TELEPORT_RECTS[PLACE_BY_ROUND[i][0]])
			
			set teleporter[i] = CreateUnit(Player(bj_PLAYER_NEUTRAL_EXTRA), ID, x, y, 0.00)
			...
			set t = CreateTrigger()
			call TriggerRegisterEnterRectSimple( t, TELEPORT_RECTS[PLACE_BY_ROUND[i][0]])
        ...
        static method update takes nothing returns nothing
            ...
            if not isActive[i] then
                //Kündige die Verschiebung des Teleporters für Forsaken-Spieler an
				loop
